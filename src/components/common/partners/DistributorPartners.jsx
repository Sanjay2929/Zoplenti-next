...
const DistributorPartners = ({ brands, show, hidden, translate_remove, margin_bottom, padding_top }) => {
    return (
        <>
            {/* Partners Section */}
            <div id="brand" className={`w-full relative sm:translate-y-[-80px] translate-y-[-40px] ${translate_remove}`}>
                <Image src="/assets/partners/brandsPartners/svg/layer-bg-image.svg" height={332} width={1440} className={`absolute min-h-[200px] top-[-20%] lg:top-[-200px] xl:top-[-226px] start-0 w-full ${hidden}`} alt="image" />
                <div className={`container md:px-12 xl:px-3 sm:px-8 max-w-[1140px] relative z-10 px-4 mx-auto pb-[60px] sm:pb-20 md:pb-[100px] lg:pb-[120px] ${margin_bottom}`}>
                    <div className="relative after:absolute after:inset-0 after:rounded-[25px] after:p-[1px] rounded-[25px] border_gradent m-1">
                        ...
                        />
                        {/* Content */}
                        <div className={`${padding_top} px-3 sm:px-6 md:px-12 lg:px-16 relative z-30 pt-11 sm:pt-[70px] pb-[34px] sm:pb-10 md:pb-11 lg:pb-12`}>
                            {show === "all" ? (
                                // HOMEPAGE PARTNERS
                                        ...
                                        return (
                                            <div key={index}>
                                                ...
                                                <div className="flex flex-auto flex-wrap sm:justify-between xl:gap-x-12 lg:gap-x-7 gap-y-6 sm:gap-y-8 md:gap-y-10 lg:gap-y-[46px] gap-x-5 md:gap-x-9">
                                                    {data.partnerslogo &&
                                                        data.partnerslogo.slice(0, 5).map((data, index) => (
                                                            <div key={index} className="flex items-center">
                                                                <Link href={data.url} >
                                                                    <Image
                                                                        className={`${data.style} hover:scale-[1.05] transition-all duration-700`}
                                ...
                                <div className="flex flex-auto flex-wrap justify-between xl:gap-x-12 lg:gap-x-7 gap-y-6 sm:gap-y-8 md:gap-y-10 lg:gap-y-[46px] gap-x-[4px] md:gap-x-9">
                                    {brands.map((data, index) => {
                                        return (
                                            <div
                                                key={index}
                                                className={`flex items-center lg:justify-center ${data.order}`}
                                            >
