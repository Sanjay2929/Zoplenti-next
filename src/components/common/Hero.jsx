      ...
      <div className="bg-midnight relative">
        <div className='pt-[90px] common_hero_layer bg-no-repeat bg-cover bg-bottom relative overflow-x-hidden'>
          ...
          <div className="relative py-[58px] container max-w-[1140px] px-4 sm:px-8 mx-auto z-20 md:my-0 md:px-12 xl:px-4 flex flex-col items-center md:items-start">
            {/* Main heading */}
            <h1 className={`font-aeoniktrials text-white text-4xl md:text-5xl lg:text-6xl font-normal text-center md:text-start !leading-[133.333%] ${props.titleStyle}`}>
              {props.title}
            </h1>
            {/* Sub heading */}
            <p className={`text-white pt-[14px] font-aeoniktrials text-sm sm:text-base md:text-lg lg:text-xl font-normal !leading-[170%] text-center md:text-start px-1 sm:px-0 ${props.descriptionStyle}`}>
              {props.description}
            </p>
            {/* Action buttons */}
